...
for i in range(len(file_names)):
    ...
    print("第%d篇文档读取完毕"%(i+1)) #全部表格的每一行（数据结构为[[table1],[table2],..[table n]],table1中为[tr1,tr2....]）
...
'''
第2张表中选取指标DB Time,AAS(DB Time/Elapsed)
'''
DB_Time = []
...
'''
第4张表选取指标 Redo Size,Logical reads,Physical reads,Physical writes,Parses,Hard parses,Logons,Transactions
'''

Logical_Reads = []

for i in range(len(table_result)):
    Logical_Reads.append(float(table_result[i][3]["Per Second"]["Logical reads:"].replace(",",'')))

Physical_Reads = []

for i in range(len(table_result)):
    Physical_Reads.append(float(table_result[i][3]["Per Second"]["Physical reads:"].replace(",",'')))

Logons = []
for i in range(len(table_result)):
    Logons.append(float(table_result[i][3]["Per Second"]["Logons:"].replace(",",'')))
...
Parses = []
for i in range(len(table_result)):
    Parses.append(float(table_result[i][3]["Per Second"]["Parses:"].replace(",",'')))

Redo_Size = []
for i in range(len(table_result)):
    Redo_Size.append(float(table_result[i][3]["Per Second"]["Redo size:"].replace(",",'')))

Physical_writes = []
for i in range(len(table_result)):
    Physical_writes.append(float(table_result[i][3]["Per Second"]["Physical writes:"].replace(",",'')))

Transactions = []
for i in range(len(table_result)):
    Transactions.append(float(table_result[i][3]["Per Second"]["Transactions:"].replace(",",'')))


'''
第6张表选取指标 Buffer Hit %,Latch Hit %,% Non-Parse CPU
'''
...
Latch_Hit = []
for i in range(len(table_result)):
    Latch_Hit.append(float(table_result[i][5][0]["Latch Hit %:"]))

Non_Parse = []
for i in range(len(table_result)):
    Non_Parse.append(float(table_result[i][5][0]["% Non-Parse CPU:"]))


import xlsxwriter

bk = xlsxwriter.Workbook("C:/users/sunchao/desktop/awr_values_Per_Second.xlsx")
...
excel_write(sh,0,3,"Logical_Reads")
excel_write(sh,0,4,"Physical_Reads")
...
for i in range(len(Logical_Reads)):
    excel_write(sh,i+1,3,Logical_Reads[i])

for i in range(len(Physical_Reads)):
    excel_write(sh,i+1,4,Physical_Reads[i])
...
create_table_sql = """\
CREATE TABLE AWR_VALUES_Per_Second(
ID int PRIMARY KEY,
TIME CHAR(20) UNIQUE,
DB_Time FLOAT(2)  ,
AAS FLOAT(2)  ,
...
"""
insert_table_sql = """\
INSERT INTO AWR_VALUES_Per_Second(ID,TIME,DB_Time,AAS,Logical_Reads,Physical_Reads,Hard_Parses)
 VALUES('{ID}','{TIME}','{DB_Time}','{AAS}','{Logical_Reads}','{Physical_Reads}','{Hard_Parses}')
"""
...
for i in range(len(Time)):
    with connection.cursor() as cursor:
        ...
        cursor.execute(insert_table_sql.format(ID=i+1,TIME=Time[i],DB_Time=DB_Time[i],
                                               AAS=AAS[i],Logical_Reads=Logical_Reads[i],
